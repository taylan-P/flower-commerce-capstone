  ...
  return (
    <div className="flex top-16 w-56 text-right">
      <Menu as="div" className="relative inline-block text-left">
        ...
        <img src={"https://via.placeholder.com/900"} alt="User Profile" />
        </button>
      </Menu.Button>
        </div>
        <Transition
        ...
        >
          <Menu.Items className="">
            <div className="px-1 py-1">
              <Menu.Item>
                {({ active }) => (
                  <button onClick={onLogout}className={`${ active ? 'bg-violet-500 text-white' : 'text-gray-900'} group flex w-full items-center rounded-md px-2 py-2 text-sm`} >
                    {active ? (
